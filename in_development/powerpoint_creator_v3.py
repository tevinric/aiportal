...
def main():
    ...
    # Sidebar Configuration (same for both modes)
        ...
        if st.session_state.mode == "prompt":
            ...
            if prompt and st.button("Generate Presentation"):
                with st.spinner("Generating presentation content..."):
                    ...
                    if st.session_state.content_data:  # Only proceed if content was generated successfully
                        if include_images:
    ...
    # Rest of the flow remains the same
        ...
        for idx, slide in enumerate(st.session_state.content_data["slides"]):
            ...
            with st.spinner(f"Generating image {idx + 1}/{total_slides}..."):
                try:
                    prompt = f"Create a suitable image for slide titled '{slide['title']}' with content: {slide['content']}"
        ...
        if st.button("Create New Presentation"):
            for key in ["content_data", "current_step", "mode", "image_descriptions", "generated_images"]:
