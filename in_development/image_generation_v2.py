...
def improve_prompt_based_on_feedback(original_prompt, satisfaction, aspects, feedback_text, current_settings):
    """
    Use GPT-4 to improve the prompt based on user feedback.
    """
    try:
        # Construct detailed feedback context
        feedback_context = f"""
        Original prompt: {original_prompt}
        User satisfaction level: {satisfaction}
        Areas needing improvement: {', '.join(aspects)}
        Specific feedback: {feedback_text}
        Current style settings: {json.dumps(current_settings)}
        """

        system_message = """You are an expert at improving image generation prompts based on user feedback.
        Analyze the original prompt and user feedback to create an enhanced version that addresses the specific concerns.
        ...
        - Consider the current style settings when making improvements
        - Keep artistic integrity while incorporating feedback
        - Ensure changes align with the selected style and mood
        ...
        messages = [
            {"role": "system", "content": system_message},
            {"role": "user", "content": f"Improve this image generation prompt based on feedback: {feedback_context}"}
        ]
                ...
                if satisfaction != "Very Satisfied" and (aspects or feedback_text):
                    try:
                        with st.spinner("Improving and regenerating..."):
                            ...
                            # Generate new image
                            new_image_url = generate_image(improved_prompt)
